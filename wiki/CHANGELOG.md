...
- **Artifacts Gallery**: New `/artifacts` page showing all artifacts across all user conversations in a scrollable grid; filter tabs (All / Images / HTML / Code / Files); per-card image thumbnails with auth-aware blob fetching; click-to-preview modal with sandboxed iframe for HTML artifacts; per-artifact conversation link; `GET /api/artifacts` endpoint returns user-scoped artifact list with metadata (mime_type, size, conversation_id, conversation_title, created_at from file mtime); "Artifacts" nav item added to sidebar with `Layers` icon
...
### Changed
- **Admin Health: dedicated tab**: Integration health cards moved from Overview to a dedicated Health tab with category grouping and per-item impact warnings
