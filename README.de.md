...
*KI-gestützte Connector-Hub — als Copilot in ein System eingebettet oder alle als Hub verbunden.*
...
- **Agent-Verwaltung** — Erstellen, konfigurieren und veröffentlichen Sie Agenten mit gebundenen Modellen, Tools und Anweisungen. Pro-Agent-Ausführungsmodus (Standard/Planner) und Temperaturkontrolle. Das optionale Flag `discoverable` ermöglicht die automatische LLM-Erkennung über CallAgentTool.
- **Globale Skills (SOPs)** — Skills sind wiederverwendbare Standard Operating Procedures, die global gelten — werden für jeden Benutzer geladen, unabhängig von der Agent-Auswahl, basierend auf der Sichtbarkeit (persönlich/org/Marketplace). Im progressiven Modus (Standard) enthält der System-Prompt kompakte Stubs; das LLM ruft `read_skill(name)` auf, um bei Bedarf vollständige Inhalte zu laden und die Token-Kosten um etwa 80 % zu senken. Wenn ein Skill-SOP auf einen Agent verweist, kann das LLM über `call_agent` delegieren.
...
- **Ressourcen-Abonnements** — Benutzer durchsuchen und abonnieren freigegebene Ressourcen aus dem Marketplace. Abonnieren/Abmelden über UI oder API. Alle Ressourcentypen (Agenten, Konnektoren, Wissensdatenbanken, MCP-Server, Skills, Workflows) unterstützen Marketplace-Veröffentlichung und Abonnementverwaltung.
- **Admin-Panel** — System-Statistik-Dashboard (Benutzer, Gespräche, Token, Modellnutzungsdiagramme, Token-nach-Agent-Aufschlüsselung), Konnektoren-Aufrufmetriken (Erfolgsquote, Latenz, Aufrufe), Benutzerverwaltung mit Suche/Paginierung, Rollenwechsel, Passwort-Zurücksetzen, Kontoaktivierung/-deaktivierung und Pro-Tool-Aktivierungs-/Deaktivierungssteuerungen.
- **First-Run-Setup-Assistent** — Beim ersten Start führt Sie das Portal durch die Erstellung eines Admin-Kontos (Benutzername, Passwort, E-Mail). Dieses einmalige Setup wird zu Ihren Login-Anmeldedaten — keine Konfigurationsdateien erforderlich.
