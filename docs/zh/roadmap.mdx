...
* **连接器导入/导出**：共享连接器模板
* **连接器分叉**：复制并自定义现有连接器
* ~~**数据库连接器**：直接访问 SQL (PostgreSQL、MySQL、Oracle)~~ *(已在 v0.7.x 交付——Phase 1-3)*
...
* ~~**RBAC**：按用户/角色控制连接器访问权限~~ *(已在 v0.7.x 发布——组织系统 + 三级可见性)*
* **操作审计**：详细记录谁执行了哪些操作
...
* **连接器渐进式披露 (Phase 5)**：启发式预加载 (基于 query 预测相关连接器)；Scale 模式支持批量/ETL 连接器；CLI 化统一接口 `connector <name> <action> <params>`
