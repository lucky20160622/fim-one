...
- **Finance 副驾模式**：通过 DB 连接器接入 Kingdee (金蝶) → 查询财务报表、生成分析报告
- **Contract 副驾模式**：通过 API 连接器接入合同管理系统 → 检索合同、提取条款、评估风险
- **HR 副驾模式**：通过 API 连接器接入 HR 系统 → 查询员工信息、生成统计报表
