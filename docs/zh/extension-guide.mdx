---
title: "扩展指南"
description: "如何将自定义 LLM 提供商、工具、内存后端、嵌入、重排序器、Web 后端和 RAG 组件插入 FIM One。"
---## 概述

FIM One 围绕一组精简的抽象基类构建——每个可交换的组件对应一个基类。每个组件都有单一职责和最小化的接口。你实现抽象方法，将实例连接到适当的注册表或注入器中，系统的其余部分就会自动使用你的实现。
...
`BaseLLM` 有两个必需的方法 — `chat` 和 `stream_chat` — 以及一个可选的 `abilities` 属性，用于告诉系统该模型可以做什么。
...
`ModelRegistry` 将名称映射到 `BaseLLM` 实例，并按角色解析。系统使用四个内置角色：`general`、`fast`、`compact` 和 `vision`。您可以添加自己的角色。
...
`abilities` 字典是 LLM 和 ReAct 引擎之间的契约。当 `tool_call=True` 且 agent 使用 `use_native_tools=True` 创建时，引擎将使用原生函数调用。否则它会自动回退到 JSON 模式。
...
├── calculator.py       ← 现有工具
├── git_status.py       ← 您的新文件 → 自动发现
└── ...
```

扫描器会跳过 `_SKIP_AUTO_DISCOVER` 中列出的类。对于需要外部配置（例如 API 密钥）且需要在启动时有条件地实例化的工具，请使用该集合。### 信号不可用

重写 `availability()` 以在工具目录中显示依赖项缺失时的消息：
...
`BaseChunker` 将文本分割成 `Chunk` 对象。`MAX_CHUNK_SIZE = 6000` 字符是硬上限 — 超过此大小的块可能会溢出 Jina Embeddings v3 token 窗口。
...
几个模式在所有基类中保持一致，使得编写自定义实现更容易且正确：

**异步优先。** 每个方法都是 `async def`。即使您的实现是同步的，也应该用 `asyncio.to_thread()` 包装它，而不是阻塞事件循环。

**工具的字符串输出。** `BaseTool.run()` 返回 `str`（或 `ToolResult`）。LLM 只能看到文本——工具实现负责将复杂数据序列化为可读格式。

**最小化接口。** 每个基类定义最小的必要契约。`BaseMemory` 是三个方法；`BaseWebFetch` 是一个。您永远不需要实现不需要的功能。

**组合优于继承。** 基类是接口，不是框架。您在构造时注入实现；运行时永远不会对其进行猴子补丁或进一步子类化。