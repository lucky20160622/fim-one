...
| **Recommended** | 重要功能支持项 | 优雅降级——该功能会明确显示为不可用，但系统仍可运行 |
| **Optional** | 增强能力 | 无感降级——系统可正常运行，只是该能力不存在 |
...
| Variable | 必填 | Default | Description |
|---|---|---|---|
...
|---|---|---|---|
| `CODE_EXEC_BACKEND` | No | `local` | `local`（直接在主机上执行）或 `docker`（隔离容器） |
| `DOCKER_PYTHON_IMAGE` | No | `python:3.11-slim` | 用于 Python 执行的 Docker 镜像 |
| `DOCKER_NODE_IMAGE` | No | `node:20-slim` | 用于 Node.js 执行的 Docker 镜像 |
| `DOCKER_SHELL_IMAGE` | No | `python:3.11-slim` | 用于 shell 执行的 Docker 镜像 |
| `DOCKER_MEMORY` | No | *(Docker 默认值)* | 每个容器的 RAM 上限（例如 `256m`、`512m`、`1g`） |
| `DOCKER_CPUS` | No | *(Docker 默认值)* | 每个容器的 CPU 配额（例如 `0.5`、`1.0`） |
| `SANDBOX_TIMEOUT` | No | `120` | 默认执行超时时间（秒） |
