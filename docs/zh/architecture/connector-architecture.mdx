...
相同的连接器架构，不同的交付方式。副驾模式与枢纽使用相同的 `连接器工具适配器`。
...
```mermaid
flowchart TB
    A["<b>FIM Agent Platform</b>\nReAct / DAG / Portal UI\n<i>Multi-tenant, scheduling, conversation persistence</i>"]
    B["<b>Connector Governance Layer</b>\naudit log, auth passthrough, operation classification\n<i>read_only enforcement, confirmation gate, circuit breaker</i>"]
    C["<b>MCP Protocol (Transport)</b>\nstdio / HTTP transport\n<i>Standard tool description, process isolation</i>"]
    D["<b>Legacy System</b>\nDB / API / MQ\n<i>Client's existing systems</i>"]
...
FIM Agent&#39;s MCP Client（随 v0.3 发布）已支持第三方 MCP 服务器。遗留系统适配器本质上只是借助连接器治理层构建的**面向特定领域的 MCP 服务器**，以满足企业级治理要求。
