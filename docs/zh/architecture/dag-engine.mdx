...
**打断与取消。** 执行器有两条独立的取消路径，分别由不同事件触发：

```mermaid
flowchart LR
    A["用户发送后续消息"] -->|"exec_stop_event.set()"| B["待处理步骤 → skipped\n运行中步骤 → 正常完成"]
    C["HTTP 客户端断连"] -->|"run_task.cancel()"| D["运行中的步骤 task 被取消\nasyncio.gather 等待清理完成"]
```

*优雅跳过 — stop event。* 当用户在执行过程中发送后续消息时，`chat.py` 中的编排器设置 `exec_stop_event`。执行器在每个调度循环的顶部检查此标志：如果已设置，所有剩余的 `pending` 步骤立即标记为 `"skipped"`（原因为 `"Skipped — user changed requirements"`），执行循环退出。已在运行的步骤允许正常完成——只有未启动的步骤被放弃。这条快速退出路径让流水线无需等待原计划全部执行完毕，即可立刻围绕用户更新后的意图进行重规划。

*立即中止 — asyncio 取消。* 当 HTTP 客户端断开连接时，`chat.py` 通过 `asyncio.Task.cancel()` 取消顶层 `run_task`。执行器捕获 `asyncio.CancelledError`，取消所有正在运行的步骤 task，通过 `asyncio.gather(..., return_exceptions=True)` 等待它们确认取消，然后 re-raise 异常以向上层传播清理逻辑。客户端断连通过 SSE 事件循环中每 0.5 秒轮询一次 `await request.is_disconnected()` 来检测。

两条路径的语义差异很关键：**stop event** 意为"跳过尚未启动的步骤，但保留已在运行的步骤"——运行完成的步骤结果仍可用于重规划；**CancelledError** 意为"立即中止一切"——所有正在执行的工作均被丢弃，无法恢复结果。
