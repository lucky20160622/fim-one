...
Anthropic's API rejects `tool_choice={"type":"function","function":{"name":"X"}}` when extended thinking is enabled:

```
Thinking may not be enabled when tool_choice forces tool use
```

This is a semantic conflict at the protocol level: forcing a specific tool call contradicts the model's freedom to reason about which tool to call. Anthropic enforces this constraint; other providers do not.

`structured_llm_call` handles this by passing `reasoning_effort=None` when calling native_fc (the forced `tool_choice` level). This disables extended thinking for that specific call, so forced tool_choice always works regardless of the global reasoning configuration. Structured output calls (DAG planning, schema annotation, plan analysis) need **schema compliance**, not deep reasoning — disabling thinking here is both correct and beneficial (lower latency, lower cost).
...
Setting `json_mode_enabled=false` for Bedrock is recommended as defense-in-depth. Native_fc handles structured output reliably, but if it ever fails for an unexpected reason, disabling json_mode ensures the fallback chain skips the Bedrock prefill trap and goes directly to plain_text.
...
**"Thinking may not be enabled when tool_choice forces tool use"**
`structured_llm_call` disables thinking for native_fc calls automatically. If this error appears, it indicates a code path that bypasses `structured_llm_call` or a custom `BaseLLM` subclass that does not accept the `reasoning_effort` kwarg. Check that your LLM subclass signature matches `BaseLLM.chat()`.
