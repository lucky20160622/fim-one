...
**Objectif** : Opérations de qualité production, débogage et surveillance. Introduit le **Système de hooks** — une couche d'application déterministe qui se situe sous les instructions de l'agent et ne peut pas être remplacée par le LLM.
...
- [ ] **Intégration des canaux IM (bidirectionnelle)** : **Phase 1 — Push sortant** : actions de notification Lark, WeCom, Slack, Email, Teams à partir des résultats d'Agent/Workflow. **Phase 2 — Déclenchement entrant** : les utilisateurs @mentionnent l'Agent dans les chats de groupe IM pour déclencher des tâches sans ouvrir le Portail ; récepteur webhook par canal ; chaque canal IM modélisé comme un connecteur avec actions bidirectionnelles (envoi + réception). Fonctionnalité phare du mode Hub

#### API Public (Phase 2)

Phase 1 (livrée) : middleware d'authentification par clé API, support des portées, spécification OpenAPI organisée, Référence API Mintlify avec terrain de jeu interactif.
...
- [ ] **Application des portées par endpoint** — Dépendance `require_scope("chat")` sur tous les endpoints protégés ; les clés avec `scopes=chat` ne peuvent accéder qu'aux API liées au chat
- [ ] **Versioning API** (`/v1/...`) — Contrat API versionné stable ; en-têtes de dépréciation pour les endpoints en fin de vie
- [ ] **Rappels webhook** — Enregistrer les URL webhook par clé API ; recevoir des notifications POST pour la fin de conversation, les erreurs d'agent et les résultats de tâches asynchrones
...
- [ ] **Portail développeur** — Panneaux interactifs « Essayer » dans la documentation Mintlify ; analytique d'utilisation visible aux propriétaires de clés
- [ ] **Rotation de clé API** — Rotation de clé en un clic avec période de grâce (ancienne clé valide 24h après rotation)
- [ ] **API par lot / asynchrone** — `POST /api/batch` acceptant jusqu'à 100 requêtes ; retourne un `batch_id` pour interroger les résultats ; utile pour les requêtes KB en masse ou l'orchestration multi-agent
- [ ] **Disjoncteur par dépendance externe** — Prévenir les défaillances en cascade quand les fournisseurs LLM ou connecteurs en aval sont indisponibles ; basculement automatique et récupération

- [ ] **Couche de trace d'agent (Observabilité++)** : hiérarchie run/trace/thread au niveau application pour le débogage d'agent — chaque conversation → `Trace`, chaque appel LLM / appel d'outil / étape DAG → `Span` avec entrée/sortie/jetons/timing. Visionneuse de trace frontend avec chronologie et charges utiles d'appel LLM extensibles. Cela va au-delà d'OTel (niveau infrastructure) pour fournir un débogage de boucle d'agent exploitable pour les développeurs et les clients d'entreprise. Export OpenTelemetry comme option de récepteur de données. Modélisé d'après les concepts run/trace/thread de LangSmith — le modèle validé par l'industrie pour l'observabilité d'agent.
- [ ] **Tableau de bord des métriques** : latence, taux de succès, utilisation de jetons, analytique d'appels de connecteur — par agent, par utilisateur, par organisation
- [x] ~~**Disjoncteur** : machine à trois états (fermé/ouvert/semi-ouvert) avec suivi des défaillances par connecteur, détection 5xx et endpoints de surveillance~~ *(livré en avance — implémenté en v0.8)*
- [x] **Nettoyage de rétention des exécutions de workflow** : tâche de nettoyage en arrière-plan avec âge maximal configurable et nombre maximal par workflow ; remplacements par workflow ; endpoint administrateur pour déclenchement manuel
- [x] **Résumés de changement de version de workflow** : `compute_blueprint_diff()` génère automatiquement des résumés lisibles par l'homme à partir des diffs de blueprint lors de la sauvegarde de version
- [ ] **Auto-abonnement aux dépendances de connexion de workflow** : Étendre la cascade d'abonnement Market pour auto-abonner les dépendances de connexion du workflow (connecteurs API, serveurs MCP). Les nœuds de workflow peuvent référencer des connecteurs, serveurs MCP, agents (qui référencent récursivement plus de dépendances) et sous-workflows — toutes les dépendances de connexion dans l'arborescence complète doivent être auto-abonnées à l'abonnement et nettoyées en cascade à la désinscription. Plus complexe que Skill/Agent en raison de la résolution récursive de sous-workflow avec détection de cycle. Équivalent à l'auto-abonnement aux dépendances de connexion de Solution (Skill/Agent)
- [x] **Exécuteurs de workflow réels** : remplacé les stubs d'exécuteur de nœud MCP et BuiltinTool par des implémentations complètes (découverte de serveur MCP + appel d'outil ; intégration ToolRegistry)
...
- [ ] **Espace de travail d'agent (Bureau d'agent persistant)** : Trois couches : (1) **Déchargement de sortie d'outil** — auto-sauvegarder les réponses d'outil surdimensionnées (>8K caractères) dans les fichiers `workspace://`, retourner un aperçu tronqué + URI ; outils intégrés `read_workspace_file`, `list_workspace_files`, `write_workspace_file` pour l'accès sélectif et les artefacts générés par l'agent. (2) **Notes de transfert** — `write_handoff(summary)` pour les transitions de contexte entre compression/commutations de session. (3) **Interface d'espace de travail** — panneau navigateur de fichiers frontend par conversation (aperçu texte/JSON/CSV, téléchargement, suppression/renommage) ; rétention de fichiers entre sessions ; intégration de quota de stockage par utilisateur. Étend `truncate_tool_output()` dans les adaptateurs + endpoint `GET /api/conversations/{id}/workspace`
- [x] **Injection de contenu de fichier intelligent + outil `read_uploaded_file`** : Les petits fichiers téléchargés (`<32K` caractères) sont automatiquement intégrés au contexte LLM ; les gros fichiers obtiennent des métadonnées + indice d'outil. Outil `read_uploaded_file` en mode dual (lecture paginée + recherche regex). Endpoint `GET /api/files/{file_id}/content`, stockage sidecar `.content`, `content_length` dans les réponses API de fichier
- [ ] **Traitement de document intelligent (Sensible à la vision)** : Gestion adaptative des documents basée sur les capacités du modèle — quand le LLM cible supporte la vision (Claude, GPT-4o, Gemini), envoyer les pages PDF/DOCX sous forme d'images rendues via des blocs de contenu document/image (gestion native PDF de style Claude) pour une fidélité visuelle complète (mise en page, graphiques, figures, photos). Quand le modèle est texte uniquement, revenir à l'extraction de texte pdfplumber/markitdown actuelle. Pour DOCX/PDF avec images intégrées : extraire les images séparément et les acheminer via des blocs de contenu vision au lieu de déverser base64 dans le texte. Trois niveaux : (1) **Vision en premier** — rendre les pages sous forme d'images, envoyer en tant que contenu vision (fidélité la plus élevée, coût de jeton plus élevé) ; (2) **Hybride** — extraction de texte + extraction d'image séparée pour les images intégrées (équilibré) ; (3) **Texte uniquement** — comportement pdfplumber/markitdown actuel (coût le plus bas, modèles texte uniquement). Détection de capacité de modèle via registre de fournisseur (drapeau `supports_vision`). Sélecteur de mode visible par l'utilisateur : « Précis » (vision) vs « Rapide » (extraction de texte) avec détection automatique par défaut
...
- [ ] **Rappel de conversation entre sessions** : outils d'agent pour lister, rechercher et lire l'historique de conversation passée — `list_conversations`, `search_conversations` (recherche par mot-clé/regex dans l'historique), `read_conversation` (récupérer le fil complet). Active les workflows « qu'avons-nous discuté la dernière fois » et « trouver le changement API que j'ai demandé la semaine dernière ». Associé à la gestion de fichiers entre sessions pour former une couche de mémoire d'agent à long terme complète
...
- [ ] **Fork de ressource (Phase 1 du système de package — prérequis pour le système de package v1.0)** : Endpoints de clone/fork par ressource comme blocs de construction atomiques pour le fork de package. Chaque `POST /api/{type}/{id}/fork` crée une copie profonde de la configuration de ressource appartenant à l'utilisateur, découplée de l'original (pas de lien de mise à jour). **Matrice de test** : chaque ressource doit être testée à la fois pour l'abonnement au marché (installation) et la publication au niveau organisation (deux chemins de code distincts). Ordre d'implémentation par complexité :
  1. **Fork de serveur MCP** — le plus simple ; copier la configuration (commande, arguments, modèle d'env). L'override de credential MCP par utilisateur (`mcp_server_credentials`) fournit déjà l'isolation de credential atomique — étendre ce modèle
  ...
  4. **Fork de connecteur** — déjà livré (v0.8) ; vérifier qu'il gère l'élimination des credentials au fork (l'utilisateur doit fournir ses propres credentials)
  5. **Fork de workflow** — la duplication existe déjà ; vérifier qu'elle gère les références de nœud aux agents/connecteurs/KBs
  6. **Fork de KB** — le plus complexe ; **copie superficielle uniquement** (métadonnées + références de document, embeddings régénérés). La copie profonde d'indices vectoriels est prohibitivement coûteuse et gaspilleuse puisque les utilisateurs de fork modifient généralement les documents. Référencer l'approche de npm : ne pas sur-concevoir, superficiel est suffisant
  **Stratégie de câblage de fork** : tri topologique sur le graphe de dépendance de ressource — fork les nœuds feuille en premier (KB, Connecteur, MCP), construire une table de mappage `old_id → new_id`, puis fork les nœuds parents (Agent, Skill) avec substitution d'ID. Réutiliser les modèles du résolveur de conflit d'import de workflow (`compute_blueprint_diff()` mappage d'ID). Cette Phase
