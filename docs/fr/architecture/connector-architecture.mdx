---
title: "Architecture du connecteur"
description: "Comment FIM One connecte les systèmes hérités via l'IA — du Copilot au Hub."
---## Copilot vs Hub

L'architecture supporte deux échelles d'intégration :
...
Même architecture de connecteur, livraison différente. Un Copilot utilise le même `ConnectorToolAdapter` qu'un Hub.## Principe fondamental

**Le client ne change aucun code.** FIM One s'intègre de manière proactive dans leurs systèmes -- en lisant leurs bases de données, en appelant leurs API, en poussant vers leur bus de messages. Le client fournit uniquement les identifiants et l'accès réseau.## Architecture à trois couches
...
| Couche | Gère | Change quand... |
|---|---|---|
| **Platform** | Orchestration, multi-tenant, UI | De nouvelles fonctionnalités de plateforme sont déployées |
| **Connector Governance Layer** | Politiques de gouvernance d'entreprise | Les exigences de sécurité/conformité changent |
| **MCP Protocol** | Transport, standard d'interface d'outil | Jamais (standard ouvert) |
| **Legacy System** | Données métier et logique | Jamais (c'est tout l'intérêt) |## Pourquoi MCP comme couche de transport
...
- **Écosystème** : Les serveurs MCP tiers (bases de données, API, outils SaaS) fonctionnent directement.
- **Isolation des processus** : Chaque serveur MCP s'exécute en tant que processus séparé. Un adaptateur défaillant ne peut pas faire planter la plateforme.### Ce que MCP seul ne fournit pas
...
|---|---|---|
| Application de lecture seule | Non | Drapeau `read_only` sur les opérations ; écriture bloquée par défaut |
...
| Porte de confirmation | Non | Les opérations d'écriture nécessitent l'approbation humaine (SSE `confirmation_required`) |
| Disjoncteur | Non | L'échec de la connexion déclenche une dégradation gracieuse |
| Classification des opérations | Non | Opérations étiquetées comme lecture/écriture/admin avec des politiques par niveau |### Pourquoi ne pas inventer un protocole personnalisé

Le protocole est une commodité. La valeur technique réside dans les adaptateurs eux-mêmes (connaissance du domaine, mappage de schéma, gestion des cas limites) et la couche de gouvernance (audit, authentification, sécurité). Inventer un protocole de transport ajouterait un coût de maintenance sans ajouter de capacité. Stripe utilise HTTPS ; Docker utilise cgroups ; FIM One utilise MCP.## Modèle de déploiement
...
| Base de données + bus de messages | Adaptateur de base de données + adaptateur de push de messages |## Découplage Agent-Connecteur

L'agent voit les connecteurs comme des outils ordinaires. Il ne sait pas et ne se soucie pas de savoir si un outil est intégré, un serveur MCP tiers ou un connecteur système hérité.
...
- Le même agent peut utiliser des outils intégrés et des connecteurs dans une seule tâche.## Évolution du Hot-Plug

| Version | Comment ajouter un nouveau connector | Redémarrage requis ? |
|---|---|---|
| **v0.6** | Écrire un serveur MCP Python avec Connector Governance Layer, ajouter à docker-compose | Redéployer |
| **v0.8** | Écrire une config YAML/JSON, la plateforme génère le serveur MCP | Redémarrer |
| **v1.0** | Télécharger une spec OpenAPI, l'IA génère la config automatiquement | **Aucun redémarrage (hot-plug)** |

Les déploiements Enterprise sont « implémenter une fois, exécuter pendant des mois » -- le hot-plug est une commodité de v1.0, pas une exigence de v0.6.## Exemple de flux de données

Utilisateur : « Vérifier tous les contrats en retard du système financier et envoyer un résumé à Lark. »

```
1. L'utilisateur envoie un message via Portal / API

2. FIM One (mode ReAct):
...
| **Niveau 2** | v0.8 | Configuration YAML/JSON, génération automatique du serveur MCP par la plateforme | Ingénieur d'implémentation (Python non nécessaire) |
| **Niveau 3** | v1.0 | Télécharger la spécification OpenAPI/Swagger, l'IA génère la configuration | IA (avec révision humaine) |## Relation avec l'écosystème MCP existant
