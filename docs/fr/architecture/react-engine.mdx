...
Les chaînes de raisonnement longues (10+ appels d'outils) risquent une **dérive d'objectif** — l'agent déplace progressivement son attention de l'objectif initial vers un sous-problème local, répète des actions similaires ou entre dans des boucles de retry circulaires. L'auto-réflexion en cours d'exécution est une contre-mesure légère.

Tous les `_SELF_REFLECTION_INTERVAL` itérations d'appels d'outils (par défaut : **6**), l'agent injecte un message utilisateur dans la conversation demandant au LLM de faire une pause et de réfléchir :
...
Cela coûte ~100 tokens par injection (pas d'appel LLM supplémentaire) et n'a aucun effet sur les exécutions courtes (`< 6` appels d'outils). Il complète ContextGuard (qui gère les budgets de tokens) et la vérification par étape (qui valide les résultats individuels) en abordant un mode de défaillance différent : l'agent perdant de vue son objectif sur de nombreuses itérations.
