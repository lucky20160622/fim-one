...
</Tip>## Option A : Docker (Recommandé)
...
Les données sont persistantes dans les volumes nommés Docker (`fim-data`, `fim-uploads`) et survivent aux redémarrages des conteneurs.
...
cp example.env .env
```# Modifier .env : définir LLM_API_KEY# Installer
uv sync --all-extras
...
| `./start.sh api` | FastAPI uniquement (sans interface, pour intégration ou test)     | http://localhost:8000/api                |## Configuration
...
```# LLM_BASE_URL=https://api.openai.com/v1   # par défaut — à modifier pour d'autres fournisseurs# LLM_MODEL=gpt-4o                         # default — change for other models

JINA_API_KEY=jina_...                       # unlocks web tools + RAG
```

Pour une liste complète de toutes les options de configuration, consultez la référence [Variables d'environnement](/configuration/environment-variables).## Déploiement en Production### Docker (Recommandé)

`docker compose up -d` lance tout ce dont vous avez besoin — **aucune configuration manuelle de service requise** :
...
Avec `WORKERS=1` (par défaut), aucun Redis ou PostgreSQL n'est nécessaire — SQLite fonctionne bien.#### Proxy inverse Nginx
...
L'API s'exécute en interne sur le port 8000 — Next.js proxifie automatiquement les requêtes `/api/*`. Seul le port 3000 doit être exposé.#### Bac à sable d'exécution de code
...
```# Démarrer une instance Redis (Docker ou package système)
docker run -d --name redis -p 6379:6379 redis:7-alpine# Ajouter à .env
