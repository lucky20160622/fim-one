...
- **Organization system**: Full CRUD for organizations with role-based membership (owner/admin/member); auth helpers (`get_user_org_ids`, `require_org_member/admin/owner`); admin management at `/api/admin/organizations`; frontend admin-organizations component with member management UI
- **Three-tier resource visibility**: New `visibility` column (personal/org/global) + `org_id` on agents, connectors, knowledge bases, and MCP servers; unified `build_visibility_filter()` query helper; resources visible to user = personal + org-shared + global
- **Publish/unpublish API**: All four resource types gain `POST /api/{resource}/{id}/publish` and `POST /api/{resource}/{id}/unpublish` endpoints for org/global sharing
- **Owner delegation**: Published agents transparently access the owner's connectors and knowledge bases in chat, so consumers do not need their own credentials
- **Admin set-visibility endpoint**: Replaces the legacy clone-to-global workflow; admin can set any resource's visibility directly
- **Organization management UI**: Admin panel organization table with create/edit/delete dialogs, member management (add/remove/role change), and full i18n (EN + ZH)

### Changed
- Resource list queries now use unified visibility filter (personal + org-shared + global) via `build_visibility_filter()`
- Global resources now use a reference model instead of clone model — source resource is published directly with `visibility='global'`
- Response schemas for agents, connectors, knowledge bases, and MCP servers now include `visibility` and `org_id` fields

### Fixed
- Legacy clone agents (`user_id=NULL`) resolved to reference model via data migration backfill
- SQLite boolean literal handling in visibility migration backfill
- Duplicate `get_user_org_ids` helper in auth.py removed
