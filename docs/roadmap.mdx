...
**Goal**: Production-grade operations, debugging, and cross-session intelligence.
...
- **Cross-Session Long-Term Memory**: user-scoped persistent memory that remembers key facts, preferences, and decisions across conversations; includes importance scoring, semantic retrieval with temporal decay, and silent end-of-session summarization; integrates with existing RAG embedding pipeline
  - `user_memories` table (ORM + Alembic migration) with embedding storage
  - Memory write agent: importance scoring (1–5), auto-categorization
  - Semantic retrieval at conversation start (inject into system prompt)
  - Temporal decay (30-day half-life, recent memories weighted higher)
  - Manual trigger keywords ("remember this", "记住这个")
  - Memory management UI (view / edit / delete memories)

**Impact**: Run FIM Agent at scale with confidence. Users benefit from persistent context that makes every conversation smarter than the last.
...
- **Memory lifecycle v2**: TTL policies, admin-managed memory quotas, organization-wide shared memories (builds on v0.9 cross-session memory)
...
| ~~Semantic Memory Store~~ | **Planned for v0.9** as Cross-Session Long-Term Memory |
| ~~Memory Lifecycle~~ | **Planned for v0.9** (temporal decay, importance scoring) + **v1.0** (TTL policies, admin quotas) |
