...
- [x] **DAG quality overhaul**: default model upgrade for non-fast steps; skill auto-discovery in planning; citation verifier for legal/medical/financial domains; structured content context preservation; domain classification in router with domain-aware model selection
- [x] **Domain model escalation in ReAct**: specialist domains auto-escalate to reasoning model with mandatory web search and citation verification
- [x] **Per-model Native Function Calling toggle**: `tool_choice_enabled` setting lets models skip forced tool selection and go directly to JSON Mode
- [x] **DatabaseMetaTool (Progressive Disclosure for DB connectors)**: single `database` meta-tool with `list_tables`/`discover`/`query` subcommands replaces 3N individual tools per database connector; configurable via `DATABASE_TOOL_MODE` env var (`progressive` default, `legacy` fallback)
- [x] **On-demand tool loading via `request_tools` meta-tool**: when >12 tools are available after smart selection, LLM can dynamically load additional tools mid-conversation; works in both JSON and native function-calling modes
- [x] **MCPServerMetaTool (Progressive Disclosure for MCP)**: single `mcp` meta-tool with `discover`/`call` subcommands replaces N*M individual MCP tools; configurable via `MCP_TOOL_MODE` env var (`progressive` default, `legacy` fallback)
...
- [x] **ReAct Cycle Detection** — Deterministic duplicate tool call detection with configurable threshold; prevents agent loops without relying on LLM self-awareness
- [x] **ReAct Completion Checklist** — One-time pre-answer verification prompt when tools were used; reduces premature conclusions
#### Ecosystem & Scaling
...
- [ ] **Resource Fork (Package Phase 1 — prerequisite for v1.0 Package System)**: Per-resource clone/fork endpoints as atomic building blocks for Package fork. Each `POST /api/{type}/{id}/fork` creates a user-owned deep copy of the resource config, decoupled from the original (no update link). **Testing matrix**: each resource must be tested for both Market subscribe (install) and org-level publish (two distinct code paths). Implementation order by complexity:
  1. ~~**MCP Server fork**~~ — **done**; copies config, skips encrypted env/headers, sets `visibility=personal`, tracks `forked_from` lineage. 41 tests (shared with Skill fork)
  2. ~~**Skill fork**~~ — **done**; copies name/description/content/script/resource_refs, skips publish status, sets `visibility=personal`, tracks `forked_from` lineage
