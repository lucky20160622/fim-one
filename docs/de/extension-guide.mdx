...
| Tool | `BaseTool` | `core/tool/base.py` | Datei in `builtin/` ablegen |
...
| Bildgenerierung | `BaseImageGen` | `core/image_gen/base.py` | Constructor Injection |
...
Tools sind die häufigste Erweiterung. `BaseTool` hat drei erforderliche Komponenten: `name`, `description` und `run`. Alles andere hat sinnvolle Standardwerte.
...
Der Scanner überspringt Klassen, die in `_SKIP_AUTO_DISCOVER` aufgelistet sind. Verwenden Sie diesen Satz für Tools, die externe Konfiguration erfordern (z. B. einen API-Schlüssel) und bei Startup bedingt instanziiert werden müssen.### Signalisierung von Nichtverfügbarkeit

Überschreiben Sie `availability()`, um eine Nachricht im Tool-Katalog anzuzeigen, wenn eine Abhängigkeit fehlt:
...
Geben Sie ein `ToolResult` statt eines einfachen `str` zurück, wenn Ihr Tool Dateien produziert:
...
`BaseReranker` nimmt eine Abfrage und eine Liste von Dokumentzeichenfolgen entgegen und gibt sie neu geordnet mit Scores zurück.
...
---## Benutzerdefinierte Web-Backends### Web-Abruf
...
Einige Muster sind konsistent über alle Basisklassen hinweg, die es einfacher machen, benutzerdefinierte Implementierungen korrekt zu schreiben:
...
**Textausgabe von Tools.** `BaseTool.run()` gibt `str` (oder `ToolResult`) zurück. Das LLM sieht nur Text — Tool-Implementierungen sind dafür verantwortlich, komplexe Daten in ein lesbares Format zu serialisieren.
