...
| Reverse-Proxy (gleiche Domain) | `https://yourdomain.com/api/auth/oauth/github/callback` |## GitHub
...
4. Fügen Sie unter **Authorized redirect URIs** hinzu:
   ...
   ```
5. Klicken Sie auf **Create**, dann kopieren Sie die **Client ID** und **Client Secret**.

**Automatisch angeforderte Scopes:** `openid`, `email`, `profile`

> Hinweis: Während der Entwicklung zeigt der Consent Screen eine Warnung "This app is not verified" an. Sie können diese für Tests überspringen. Veröffentlichen Sie den Consent Screen, wenn Sie die Warnung für Endbenutzer entfernen möchten.
...
3. Klicken Sie unter **Redirects** auf **Add Redirect** und geben Sie ein:
   ```
...
<Warning>
**Nur Mandanten-Bereich.** Feishu-interne Apps (自建应用) können nur Benutzer innerhalb des **gleichen Enterprise-Mandanten** authentifizieren, der die App erstellt hat. Benutzer von anderen Feishu-Mandanten sehen eine Fehlermeldung „keine Berechtigung". Das bedeutet, dass Feishu OAuth **nicht für öffentliche SaaS-Bereitstellungen geeignet ist**, bei denen Benutzer aus verschiedenen Organisationen stammen. Es funktioniert gut für **private / Einzelorganisations-Bereitstellungen**, bei denen alle Benutzer zum gleichen Feishu-Mandanten gehören.

Um mandantenübergreifendes Feishu-Login zu unterstützen, müssten Sie die App im [Feishu App Directory](https://open.feishu.cn/document/store-v1/overview) (应用商店) veröffentlichen und Feishus Überprüfungsprozess durchlaufen — was komplex ist und strenge Anforderungen hat.
</Warning>

**Entwicklerkonsole:** https://open.feishu.cn/app

1. Klicken Sie auf **Create App** → **In-house app** (自建应用). Unternehmens- oder Enterprise-Konten sind erforderlich — Feishu OAuth ist nicht für persönliche Konten verfügbar.
...
**Scopes konfiguriert in der Konsole (nicht in der Autorisierungs-URL):** `contact:user.email:readonly`

> Hinweis: Feishu verwendet `App ID` / `App Secret` statt der Standard-Benennung `Client ID` / `Client Secret`. Die Umgebungsvariablennamen spiegeln dies wider: `FEISHU_APP_ID` und `FEISHU_APP_SECRET`.
...
Die Feishu-Benutzerinformations-API (`authen/v1/user_info`) gibt die **Enterprise-Kontakt-E-Mail** des Benutzers (联系邮箱) zurück — die Adresse, die vom Organisationsadministrator in der Feishu-Administrationskonsole konfiguriert wurde. Dies ist **nicht** die persönliche Feishu-Login-E-Mail des Benutzers.
...
- Benutzer mit persönlichen Feishu-Konten (nicht Teil eines Unternehmens) erhalten immer eine leere E-Mail.
...
Die vom OAuth-Provider zurückgegebene E-Mail muss mit der registrierten E-Mail des FIM-Kontos übereinstimmen. Dies verhindert, dass versehentlich das Social-Media-Konto einer anderen Person mit Ihrem FIM-Konto verknüpft wird.

Wenn die E-Mails nicht übereinstimmen, wird die Bindung mit einem `email_mismatch`-Fehler abgelehnt.### Feishu-Ausnahme

Da die API von Feishu die E-Mail des Unternehmenskontakts anstelle der Anmelde-E-Mail zurückgibt und dieses Feld für persönliche Konten oder Konten in Organisationen, in denen der Administrator es nicht konfiguriert hat, oft leer ist, folgt die E-Mail-Abgleichsprüfung für Feishu einer lockereren Regel:

- **Beide Seiten haben eine E-Mail** → die E-Mails müssen übereinstimmen (gleiche Prüfung wie bei anderen Anbietern).
- **Eine Seite hat keine E-Mail** (Feishu hat leer zurückgegeben oder das FIM-Konto hat keine E-Mail) → die Bindung erfolgt basierend auf der ticketgestützten Authentifizierung allein.### Feishu-Anmeldung (noch nicht gebunden)
...
3. Wenn keine Übereinstimmung vorhanden ist (auch wenn die E-Mail leer ist), wird ein neues FIM-Konto erstellt, das durch die Feishu `open_id` identifiziert wird. Das Konto hat eine null E-Mail, die der Benutzer später in den Einstellungen ausfüllen kann.## Umgebungsvariablen – Kurzreferenz

| Variable | Provider | Erforderlich | Beschreibung |
|---|---|---|---|
