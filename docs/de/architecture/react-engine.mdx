...
Lange Reasoning-Ketten (10+ Tool-Aufrufe) bergen das Risiko der **Zielabweichung** — der Agent verschiebt allmählich seinen Fokus vom ursprünglichen Ziel zu einem lokalen Teilproblem, wiederholt ähnliche Aktionen oder gerät in zirkuläre Wiederholungsschleifen. Mid-loop Selbstreflexion ist eine leichtgewichtige Gegenmaßnahme.
...
Dies kostet ~100 Token pro Injektion (kein zusätzlicher LLM-Aufruf) und hat keine Auswirkung auf kurze Läufe (`< 6` Tool-Aufrufe). Es ergänzt ContextGuard (das Token-Budgets verwaltet) und die Verifikation pro Schritt (die einzelne Ergebnisse validiert), indem es einen anderen Fehlermodus adressiert: dass der Agent über viele Iterationen hinweg sein Ziel aus den Augen verliert.
