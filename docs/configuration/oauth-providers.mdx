...
FIM Agent supports social login via GitHub, Google, Discord, and Feishu (Lark). Any provider whose credentials are present in the environment automatically shows a login button on the sign-in page — no code changes required.
...
FIM Agent handles this gracefully: see [Account Binding Rules](#account-binding-rules) below.
